...
<!-- LIST DATA -->
<x-list-data judul="Data Transaksi Pembayaran" tambah="true" :thead="array('No Bayar','NIS','Tanggal','Keterangan','Periode','Status','','Action')" :thwidth="array(15,15,15,15,15,15,15,15)" :thclass="array('','','','','','','','text-center')" />
<!-- END LIST DATA -->
                                ...
                                <div class="form-group col-md-12 col-sm-12">
                                    <div class="row">
                                        <div class="col-md-3 col-sm-12">
                                            <label for="no_bayar" id="no_bayar_html">No Tagihan</label>
                                            <div class="input-group">
                                                <div class="input-group-prepend hidden" style="border: 1px solid #d7d7d7;">
                                                    <span class="input-group-text info-code_no_bayar" readonly="readonly" title="" data-toggle="tooltip" data-placement="top" ></span>
                                                </div>
                                                <input readonly type="text" class="form-control inp-label-no_bayar" id="no_bayar" name="no_bayar" value="" title="">
                                                <span readonly class="info-name_no_bayar hidden">
                                                    <span></span> 
                                                </span>
                                                <i readonly class="simple-icon-close float-right info-icon-hapus hidden"></i>
                                                <i readonly class="simple-icon-magnifier search-item2" id="search_no_bayar"></i>
                                            </div>
                                        </div>
                                        <div class="col-md-3 col-sm-12">
                                    ...
                                    <div class='col-xs-12' style='min-height:420px; margin:0px; padding:0px;'>
                                        ...
                                        <thead style="background:#F8F8F8">
                                            <tr>
                                                ...
                                                <th style="width:25%">Keterangan Tagihan</th>
...
<script>
    ...
    //SHOW SEARCH-MODAL
    function showFilter(param,target1,target2,target4){
        var par = param;
        console.log(par);
        var modul = '';
        var header = [];
                    ...
                    $.ajax({
                        ...
                        success:function(res){
                            ...
                            for(var j=0; j<result.detail.length; j++){
                                total += parseInt(result.detail[j].nilai);
                            }
                            // console.log(total);  
                            $($target4).val(total);
                            $($target4).attr('value',total);
                        }
                    }); 
                }
                if($target3 != ""){
    ...
    $('#saku-datatable').on('click', '#btn-edit', function(){
        ...
        $('#no_bayar_html').parent().removeClass('hidden');
        $('#search_no_bayar').addClass('hidden');
        $iconLoad.show();
        $.ajax({
            type: 'GET',
                ...
                if(result.status){
                    ...
                    if(result.detail.length > 0){
                        ...
                        for(var i=0;i<result.detail.length;i++){
                            ...
                            input += "<td ><span class='td-jenis tdjeniske"+no+" tooltip-span'>"+line.keterangan+"</span><input type='text' name='ket_tagihan[]' class='form-control inp-jenis jeniske"+no+" hidden'  value='"+line.keterangan+"' readonly></td>";
                            input += "<td class='text-right'><span class='td-nilai-t tdniltke"+no+" tooltip-span'>"+parseInt(line.nilai_t)+"</span><input type='text' name='nilai_t[]' class='form-control inp-nilai-t niltke"+no+" hidden'  value='"+parseInt(line.nilai_t)+"' readonly></td>";
                            input += "<td class='text-right'><span class='td-nilai-b tdnilbke"+no+" tooltip-span'>"+parseInt(line.nilai_b)+"</span><input type='text' name='nilai_b[]' class='form-control inp-nilai-b nilbke"+no+" hidden'  value='"+parseInt(line.nilai_b)+"' required></td>";
    ...
    $('#saku-datatable').on('click', '#btn-tambah', function(){
        ...
        $('#no_bayar_html').parent().addClass('hidden');
    ...
    $('#table-data tbody').on('click','td',function(e){
        if($(this).index() != 6){
            var id = $(this).closest('tr').find('td').eq(0).html();
            var nimSiswa = $(this).closest('tr').find('td').eq(1).html();
            ...
            $.ajax({
                ...
                success:function(res){
                    ...
                    if(result.status){
                        ...
                        var html = 
                        `<tr>
                            <td style='border:none'>Nomor Pembayaran</td>
                                ...
                                <table id='table-ju-preview' class='table table-bordered'>
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width:25%">Keterangan Tagihan</th>
                                            <th style="width:20%">Nilai Tagihan</th>
                                            <th style="width:20%">Nilai Bayar</th>                                        </tr>
                                    </thead>
                                    <tbody>
                                    </tbody>
                                </table>
                        ...
                        if(result.detail.length > 0){
                            ...
                            for(var i=0; i<result.detail.length; i++){
                                ...
                                input += "<td class='text-right'>"+line.nilai_t+"</td>";
                                input += "<td class='text-right'>"+line.nilai_b+"</td>";
                                input += "</tr>";
                                no++;
                            }
                ...
                $.ajax({
                    ...
                    success:function(result){
                        if(result.data.status){
                            ...
                            $('[class*=inp-label-]').attr('style','border-top-left-radius: 0.5rem !important;border-bottom-left-radius: 0.5rem !important;border-left:1px solid #d7d7d7 !important');
                            hitungTotalRow();
                            console.log(result.data.no_bayar);
                            msgDialog({
                                id:result.data.no_bayar,
                                type:'simpan'
                            });
    ...
    function hideUnselectedRow() {
        $('#input-tagihan > tbody > tr').each(function(index, row) {
            if(!$(row).hasClass('selected-row')) {
                ...
                var nilai_t = $('#input-tagihan > tbody > tr:eq('+index+') > td').find(".inp-nilai-t").val();
                var nilai_b = $('#input-tagihan > tbody > tr:eq('+index+') > td').find(".inp-nilai-b").val();
                ...
                $('#input-tagihan > tbody > tr:eq('+index+') > td').find(".inp-nilai-t").val(nilai_t);
                $('#input-tagihan > tbody > tr:eq('+index+') > td').find(".td-nilai-t").text(nilai_t);
                $('#input-tagihan > tbody > tr:eq('+index+') > td').find(".inp-nilai-b").val(nilai_b);
                $('#input-tagihan > tbody > tr:eq('+index+') > td').find(".td-nilai-b").text(nilai_b);
                ...
                $('#input-tagihan > tbody > tr:eq('+index+') > td').find(".inp-nilai-t").hide();
                $('#input-tagihan > tbody > tr:eq('+index+') > td').find(".td-nilai-t").show();
                $('#input-tagihan > tbody > tr:eq('+index+') > td').find(".inp-nilai-b").hide();
                $('#input-tagihan > tbody > tr:eq('+index+') > td').find(".td-nilai-b").show();
            }
        })
    }
    ...
    $('#input-tagihan').on('click', '.search-item', function(){
        ...
        tmp = $(this).closest('tr').find('input[name="'+par3+'"]').attr('class');
        tmp2 = tmp.split(" ");
        target4 = tmp2[2];
        
        showFilter(par,target1,target2,target4);
    });

    $('#input-tagihan').on('keydown', '.inp-kode, .inp-jenis, .inp-nilai-t, .inp-nilai-b', function(e) {
        var code = (e.keyCode ? e.keyCode : e.which);
        var nxt = ['.inp-kode', '.inp-jenis', '.inp-nilai-t', , '.inp-nilai-b'];
        var nxt2 = ['.td-kode', '.td-jenis', '.td-nilai-t', '.td-nilai-b'];
        if (code == 13 || code == 9) {
            e.preventDefault();
    ...
    $('#form-tambah').on('click', '.add-row', function(){
        ...
        if(nim != ""){
            ...
            input += "<td class='text-right'><span class='td-nilai-t tdniltke"+no+" tooltip-span'></span><input type='text' name='nilai_t[]' class='form-control inp-nilai-t niltke"+no+" hidden'  value='' readonly></td>";
            input += "<td class='text-right'><span class='td-nilai-b tdnilbke"+no+" tooltip-span'></span><input type='text' name='nilai_b[]' class='form-control inp-nilai-b nilbke"+no+" hidden'  value='' required></td>";
            input += "<td class='text-center'><a class=' hapus-item' style='font-size:18px'><i class='simple-icon-trash'></i></a>&nbsp;</td>";
            input += "</tr>";
    ...
    $('#input-tagihan').on('click', 'td', function() {
        ...
        if (idx == 0) {
            ...
            if ($(this).hasClass('px-0 py-0 aktif')) {
                ...
                var nilai_t = $(this).parents("tr").find(".inp-nilai-t").val();
                var nilai_b = $(this).parents("tr").find(".inp-nilai-b").val();
                ...
                $(this).parents("tr").find(".inp-nilai-t").val(nilai_t);
                $(this).parents("tr").find(".td-nilai-t").text(nilai_t);
                $(this).parents("tr").find(".inp-nilai-b").val(nilai_b);
                $(this).parents("tr").find(".td-nilai-b").text(nilai_b);
                if (idx == 3) {
                    $(this).parents("tr").find(".inp-nilai-t").show();
                    $(this).parents("tr").find(".td-nilai-t").hide();
                    $(this).parents("tr").find(".inp-nilai-t").focus();
                } else {
                    $(this).parents("tr").find(".inp-nilai-t").hide();
                    $(this).parents("tr").find(".td-nilai-t").show();
                }
                if (idx == 4) {
                    $(this).parents("tr").find(".inp-nilai-b").show();
                    $(this).parents("tr").find(".td-nilai-b").hide();
                    $(this).parents("tr").find(".inp-nilai-b").focus();
                } else {
                    $(this).parents("tr").find(".inp-nilai-b").hide();
                    $(this).parents("tr").find(".td-nilai-b").show();
                }
                hitungTotalRow();
                var nama_jenis = $(this).parents("tr").find(".inp-jenis").val();
