            ...
            if(result.status){
                switch(index){
                    ...
                    if(result.data[0].foto == "-" || result.data[0].foto == "" || result.data[0].foto == undefined){
                        ...
                        var foto = "{{ config('api.url').'dev/storage' }}/"+result.data[0].foto;
                        var img= `
                        <div class="position-absolute card-top-buttons" style="top: -15px;left: 50%;z-index: 10;opacity: ;">
                            <button id="editPhoto" alt="Edit Photo" class="btn icon-button " style="background: white;border: 1px solid #8080802b;opacity: 0.8;">
                            <i class="simple-icon-camera"></i>
                    ...
                    if(result.data[0].background == "-" || result.data[0].background == "" || result.data[0].background == undefined){
                        ...
                        var foto = "{{ config('api.url').'dev/storage' }}/"+result.data[0].background;
...
function initDash(){
    loadService('profile','GET',"dev-auth/profile"); 
}
initDash();
...
$('#form-ubahPass').on('submit', function(e){
    e.preventDefault();
        ...
        var url = "dev-auth/update-password";
        var pesan = "saved";
...
$('.crop_image').click(function(event){
    $image_crop.croppie('result', {
        ...
        if(tipe == 'foto'){
            var toUrl = "{{ url('dev-auth/update-foto') }}";
        }else{
            var toUrl = "{{ url('dev-auth/update-background') }}";
        }
            ...
            success:function(result)
            {
                if(result.data.status){
                    ...
                    if(tipe == 'foto'){
                        ...
                        var foto = "{{ config('api.url').'dev/storage' }}/"+result.data.foto;
